...
internal sealed class BotService : BackgroundService
{
    ...
    private void RegisterEvents(SlashCommandsExtension slashCommands)
    {
        slashCommands.AutocompleteErrored += (_, args) =>
        {
            ...
            {
                _logger.LogError($"API response: {discordException.JsonMessage}");
            }
        ...
        slashCommands.SlashCommandInvoked += (_, args) =>
        {
            ...
            _logger.LogInformation($"{args.Context.User} ran slash command /{args.Context.CommandName}{optionsString}");
            return Task.CompletedTask;
        };
        ...
        slashCommands.ContextMenuInvoked += (_, args) =>
        {
            ...
            _logger.LogInformation($"{args.Context.User} invoked context menu '{args.Context.CommandName}' with resolved " +
        ...
        slashCommands.SlashCommandErrored += (_, args) =>
        {
            ...
            string? name = context.Interaction.Data.Name;
            _logger.LogError(args.Exception, $"An exception was thrown when executing slash command '{name}'");
            if (args.Exception is DiscordException discordException)
            {
                _logger.LogError("API response: {Response}", discordException.JsonMessage);
            }
